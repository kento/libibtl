...
ssize_t ibtl_dwrite(int fd, void *buf, size_t count)
{
  ...
  while (current_write_size < count) {
    ...
    fdmi_verbs_isend((char *)buf + current_write_size, chunk_size, FMI_BYTE, host_id, tag, FMI_COMM_WORLD, &req);
    fdmi_verbs_wait(&req, &stat);
...
ssize_t ibtl_read(int fd, void *buf, size_t count) 
{
  ...
  while (current_recv_size < count) {
    ...
    fdmi_verbs_irecv((char *)buf + current_recv_size, chunk_size, FMI_BYTE, host_id, tag, FMI_COMM_WORLD, &req);
    fdmi_verbs_wait(&req, &stat);
