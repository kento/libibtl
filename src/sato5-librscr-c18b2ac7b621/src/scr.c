...
static int scr_copy_xor(scr_filemap* map, const struct scr_reddesc* c, int id)
{
  ...
  int i;
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn1: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn2: %f", MPI_Wtime());
  /*KS: == end ==*/
  /* allreduce to get maximum filesize */
  unsigned long max_bytes;
  MPI_Allreduce(&my_bytes, &max_bytes, 1, MPI_UNSIGNED_LONG, MPI_MAX, c->comm);
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn3: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /* XOR Reduce_scatter */
  size_t nread = 0;
  double time_start_xor, time_start_t, time_start_r, time_start_x,  time_start_w;
  double time_end_xor, time_end_t, time_end_r, time_end_x,  time_end_w;
  double time_sum_xor=0, time_sum_t=0, time_sum_r = 0, time_sum_x = 0,  time_sum_w = 0;
  /*KS: == start ==*/
  if (scr_my_rank_world == 0) {
    time_start_t = MPI_Wtime();
  }
  /*KS: == end   ==*/
  while (nread < chunk_size) {
	...
	/*KS: == start ==*/
	if (scr_my_rank_world == 0) {
	  time_end_r = MPI_Wtime();
	  time_sum_r += time_end_r - time_start_r;
	}
	/*KS: == end   ==*/
      } else {
	...
	time_start_xor = MPI_Wtime();
      ...
      /*KS: == start ==*/
      if (scr_my_rank_world == 0) {
	time_end_xor = MPI_Wtime();
	time_sum_xor += time_end_xor - time_start_xor;
      }
	/*KS: == end   ==*/
      if (chunk_id > 0) {
	...
	/*KS: == start ==*/
	if (scr_my_rank_world == 0) {
	  time_end_x = MPI_Wtime();
	  time_sum_x += time_end_x - time_start_x;
	}
	/*KS: == end   ==*/
      } else {
	...
	/*KS: == start ==*/
	if (scr_my_rank_world == 0) {
	  time_end_w = MPI_Wtime();
	  time_sum_w += time_end_w - time_start_w;
	}
	/*KS: == end   ==*/
      }
    }
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0) {
    time_end_t = MPI_Wtime();
    time_sum_t = time_end_t - time_start_t;
  }
  /*KS: == end   ==*/

  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn4: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn4--1: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn4-0: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn4-2: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  scr_meta_delete(meta);
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn4-3: %f", MPI_Wtime());
  /*KS: == end ==*/
  ...
  /*KS: == start ==*/
  if (scr_my_rank_world == 0)  scr_dbg(1, "scr_copy_files_tsn5: %f", MPI_Wtime());
  /*KS: == end ==*/
  /*KS: == start ==*/
  if (scr_my_rank_world == 0) {
    scr_dbg(1, "scr_copy_files_mon: read: %f secs, xor: %f secs, x: %f secs, write: %f secs, while: %f secs",
	    time_sum_r, time_sum_xor, time_sum_x, time_sum_w, time_sum_t
	    );
  }
  /*KS: == end ==*/
...
int scr_copy_files(scr_filemap* map, const struct scr_reddesc* c, int id, double* bytes)
{
  ...
  *bytes = 0.0;
  ...
  double time_start;
  double time_start_cp, time_end_cp, time_diff_cp;
  if (scr_my_rank_world == 0) {
    timestamp_start = scr_log_seconds();
    time_start = MPI_Wtime();
    time_start_cp = time_start;
  }
  ...
  if (scr_my_rank_world == 0) {
    time_end_cp = MPI_Wtime();
    time_diff_cp = time_end_cp - time_start_cp;
  }
  ...
  if (scr_my_rank_world == 0) {
    ...
    scr_dbg(1, "scr_copy_files_res: %f secs( %f secs), %e bytes, %f MB/s, %f MB/s per proc",
            time_diff, time_diff_cp,  *bytes, bw, bw/scr_ranks_world
    );
...
static int scr_flush_async_state_wait(char* state)
{
  ...
  while (!all_valid) {
    ...
    if (scr_my_rank_local == 0) {
      ...
      fprintf (stderr, "==== %s, %p\n", scr_transfer_file, hash);
      /* check for the specified state */
      scr_hash* state_hash = scr_hash_get_kv(hash, SCR_TRANSFER_KEY_STATE, state);
      fprintf (stderr, "==== state_hash:%p, stat:%s, hash:%p (%s)\n", state_hash, state, hash, scr_my_hostname);
      if (state_hash == NULL) {
        valid = 0;
      }
      /* delete the hash */
