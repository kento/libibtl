...
#include "pgz.h"
static int ckptsize = 100000000;
static size_t offset = 0;
static unsigned char *gckptbuf;
static unsigned char *gckptbuf_c;
static size_t gckptsize = 0;
static size_t gckptsize_c = 0;
...
local size_t readn2(unsigned char* addr, size_t ckptsize, unsigned char *buf, size_t len)
{
  if (offset == ckptsize) return 0;
  if (offset + len > ckptsize) {
    len = ckptsize - offset;
  }
...
local void writen2(int desc, unsigned char *buf, size_t len)
{
    memcpy(gckptbuf_c + gckptsize_c, buf, len);
    gckptsize_c += len;
}
...
local unsigned long put_header(void)
{
    ...
    if (form > 1) {                 /* zip */
        ...
        writen2(outd, head, 30);     /* write local header */
        len = 30;

        /* write file name (use "-" for stdin) */
        if (name == NULL)
            writen2(outd, (unsigned char *)"-", 1);
        else
            writen2(outd, (unsigned char *)name, strlen(name));
        ...
        writen2(outd, head, 9);      /* write extra field block */
        len += 9;
    }
    else if (form) {                /* zlib */
        ...
        head[9] = 3;                /* unix */
        writen2(outd, head, 10);
        len = 10;
        if (name != NULL)
            writen2(outd, (unsigned char *)name, strlen(name) + 1);
        if (name != NULL)
            len += strlen(name) + 1;
    }
    return len;
                       ...
                       unsigned long check, unsigned long head)
{
    ...
    if (form > 1) {                 /* zip */
        ...
        writen2(outd, tail, 46);     /* write central file header */
        cent = 46;
        ...
        writen2(outd, tail, 9);      /* write extra field block */
        cent += 9;
        ...
        writen2(outd, tail, 22);     /* write end of central directory record */
    }
    else if (form) {                /* zlib */
        PUT4M(tail, check);
        writen2(outd, tail, 4);
...
local void write_thread(void *dummy)
{
    ...
    do {
        ...
        writen2(outd, job->out->buf, job->out->len);
        drop_space(job->out);
...
local void parallel_compress(void)
{
    ...
    next->len = readn(ind, next->buf, next->size); //OK
    hold = NULL;
    dict = NULL;
    scan = next->buf;
    hash = RSYNCHIT;
...
/*On memory compresstion*/
local void parallel_compress2(unsigned char* addr, size_t ckptsize)
{
    long seq;                       /* sequence number */
    struct space *curr;             /* input data to compress */
    struct space *next;             /* input data that follows curr */
    ...
    next = get_space(&in_pool);
    // ======
    next->len = readn2(addr, ckptsize, next->buf, next->size);
...
#define DEFLATE_WRITE(flush) \
    do { \
        do { \
            ...
            strm->next_out = out; \
            (void)deflate(strm, flush); \
            writen2(outd, out, out_size - strm->avail_out); \
...
local void load_read(void *dummy)
{
	...
	// ====        
	//        in_len = len = readn2(gckptbuf, gckptsize,  in_which ? in_buf : in_buf2, BUF); 
	in_len = len = readn(ind, in_which ? in_buf : in_buf2, BUF);  // OK, used for decomp
	// ====
...
local size_t load(void)
{
...
#endif
    {
      ...
      in_left = readn(ind, in_next = in_buf, BUF); //OK used for decomp
    }
...
local void list_info(void)
{
    ...
    if (form) {
      ...
      //=====
	    ...
	    //=====
	    offset = ckptsize - 4;
	    readn2(gckptbuf, gckptsize, tail, 4);
	...
	// ====
	offset = ckptsize - 8;
	readn2(gckptbuf, gckptsize, tail, 8);            /* get trailer */
...
local void process2(char *path, unsigned char* addr, size_t data_size)
{
    int method = -1;                /* get_header() return value */
    size_t len;                     /* length of base name (minus suffix) */
    struct stat st;                 /* to get file type and mod time */
    ...
    if (decode) {
        ...
        parallel_compress2(addr, data_size);
#endif
    else
        single_compress(0);
    if (verbosity > 1) {
    ...
    //    fprintf(stderr, "======>fd3: %d\n", open("/scr/test3", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
    /* finish up, copy attributes, set times, delete original */
    if (ind != 0)
        close(ind);
    //    fprintf(stderr, "======>fd3.2: %d\n", open("/scr/test3.2", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
    if (outd != 1) {
      //        fprintf(stderr, "======>fd3.21: %d\n", open("/scr/test3.21", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
	//        if (close(outd))
	  //	    fprintf(stderr, "======>fd3.22: %d\n", open("/scr/test3.22", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
	//            bail("write error on ", out);
	//	fprintf(stderr, "======>fd3.3: %d, outd: %d\n", open("/scr/test3.3", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE), outd);
	//	fprintf(stderr, "======>fd3.4: %d\n", open("/scr/test3.4", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
        outd = -1;              /* now prevent deletion on interrupt */
	//	fprintf(stderr, "======>fd3.5: %d\n", open("/scr/test3.5", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
        if (ind != 0) {
            copymeta(in, out);
            if (!keep)
                unlink(in);
...
int run2(int argc, char **argv)
{
    ...
    for (n = 1; n < argc; n++)
	    ...
	    int ckptfd = open("./ckpt.data_o", O_RDONLY);
	    SET_BINARY_MODE(ckptfd);
	    unsigned char* ckptbuf = (unsigned char*)malloc(ckptsize);
	    gckptbuf_c = (unsigned char*)malloc(ckptsize);
	    readn(ckptfd, ckptbuf, ckptsize);
	    gckptbuf = ckptbuf;
	    gckptsize = ckptsize;
	    //	    process2(NULL, ckptbuf, ckptsize);
	    if (decode) {
	      process(strcmp(argv[n], "-") ? argv[n] : NULL);
	    } else {
	      //	      process2(strcmp(argv[n], "-") ? argv[n] : NULL, ckptbuf, ckptsize);
	      process2(NULL, ckptbuf, ckptsize);
	    }
	    writen(1, gckptbuf_c, gckptsize_c);
	    //KS: added
            done++;
        }
    option(NULL);
...
int pcompress(unsigned char** compressed, unsigned char* original, int size)
{
  ...
  process2(NULL, gckptbuf, gckptsize);
  //  fprintf(stderr, "======>fd6: %d\n", open("/scr/test6", O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE));
  *compressed = gckptbuf_c;
...
int main(int argc, char **argv) {
  ...
  compressed_size = pcompress(&compressed, ckptbuf, ckptsize);

  writen(1, compressed, compressed_size);
  fprintf(stderr, "%d -> %d bytes\n", ckptsize, compressed_size);
