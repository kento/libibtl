...
AC_DEFUN([X_AC_MACHINE], [
  ...
  AC_ARG_WITH(
    [machine-name],
    AS_HELP_STRING([--with-machine-name=NAME,Specify the type of the machine. See documentation for valid values, default=TSUBAME2]),
    [MACHINE_NAME="$withval"],
    [MACHINE_NAME="TSUBAME2"])
  ...
  if test "$MACHINE_NAME" == "TSUBAME2"; then
   scr_machine_int=$scr_tsubame2
  fi
  ...
  AC_DEFINE_UNQUOTED([SCR_TSUBAME2],[$scr_tsubame2], [Define for TSUBAME2.0 machine type.])
